...
Every task moves through the same four beats. Skills live in `skills/` and
are linked into `.cursor/skills/` so Cursor Agent can discover them.
...
- `/before-and-after` drives the `@vercel/before-and-after` CLI. Capture the
  PNGs first. `--markdown` tries 0x0.st; that host often refuses uploads.
  If it fails, put the shots on GitHub with `gh pr create` / `gh pr edit
  --attach` (needs gh 2.99+). Local paths in the PR body will not render.
...
comments, and the closing reply. Apply it to text you wrote or changed, not
to prose you didn't touch.
...
2. Run the repo's checks (see below).
...
- **Worktrees:** create under `.worktrees/` (gitignored). Branch prefix:
  `agent/`. Example:
  `git worktree add .worktrees/<task-name> -b agent/<task-name> origin/main`
- **Install in the worktree:** `npm install`
- **Dev:** `npm run dev` (Astro, default port 4321; if that port is taken,
  confirm with `lsof -i :<port>` that you are hitting this worktree)
- **Checks:** `npm run build`
- **Stack:** Astro 3 + Svelte 4. Interactive UI lives in
  `src/components/*.svelte` with `client:load` from pages.
- **Cursor skills:** `.cursor/skills/<name>` is a symlink to
  `skills/<name>`. Edit the files under `skills/`.

## Skill sources

| Skill | Source |
|---|---|
| `new-feature`, `code-structure`, `evidence-driven-testing` | `skills/` |
| `before-and-after` | vendored from vercel-labs/before-and-after |
| `greploop` | vendored from greptileai/skills |
| `greploop-apps` | local variant of greploop for huge PRs |
| `unslop` | vendored from cursor/plugins (pstack) |
